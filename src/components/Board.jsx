...
import { ItemContent } from "./Modal";
import { utilsPackage } from "../utils/utils";
import { DateContext } from "./context/DateContext";
import { useContext } from "react";
import { BoardStyled } from '../assets/css/components/BoardStyled'
import { ColumnStateIcon, ColumnStyled, ColumnTitle, StatusIcon } from "../assets/css/components/Column";
import { AlertStyled } from "../assets/css/components/AlertStyled";
import { TodoContainerStyled } from "../assets/css/components/Todos";
import { AddBtnStyled } from "../assets/css/components/ActionsStyled";

function Board() {
  const [columns, setColumns] = useState(ColumnAPI);
  const [specificItem, setSpecificItem] = useState({});
  const [isOpenCreateToDoForm, setIsOpenCreateToDoForm] = useState(false);
  ...
  const isInThePast = (date) => {
    ...
    // console.log(date.getMinutes() < today.getMinutes())
    // console.log(date.getHours(), today.getHours())
    // console.log(date.getMinutes(), today.getMinutes())
    // console.log(date.getHours() <= today.getHours() && date.getMinutes() < today.getMinutes())
    return (
  ...
  const handleEditDescription = (e) => {
    ...
    const modalContent =
      e.currentTarget.parentElement.parentElement.parentElement;
    const newDescription = modalContent.querySelector(".todoDescriptionEditingInput");
    const itemTitle = modalContent.querySelector(".modalTitle");
    const itemEditingInput = modalContent.querySelector(".todoTitleEditingInput");
  ...
  const handleEditToDo = (e) => {
    ...
    const editBtn = modalContent.querySelector(".editBtn");
    const itemEditingTitleInput =
      modalContent.querySelector(".todoTitleEditingInput");
    const itemTitle = modalContent.querySelector(".modalTitle");
    const itemDescription = modalContent.querySelector(".modalDescription");
  ...
  const handleSaveDueDate = (e) => {
    ...
    if (
      ...
      dateObj.setBackgroundState({backgroundColor: "rgb(239, 239, 239)", color: "#000"});
    }
